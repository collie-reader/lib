...
async fn fetch_feed_title_rss() {
    let title = syndication::fetch_feed_title(&fixture("hnrss-org-frontpage.rss"), None).await.unwrap();
    assert_eq!(title, "Hacker News: Front Page");
}

#[tokio::test]
async fn fetch_feed_title_atom() {
    let title = syndication::fetch_feed_title(&fixture("hnrss-org-frontpage.atom"), None).await.unwrap();
    assert_eq!(title, "Hacker News: Front Page");
}

#[tokio::test]
async fn fetch_feed_items_rss() {
    let items = syndication::fetch_feed_items(&fixture("hnrss-org-frontpage.rss"), None).await.unwrap();
    assert_eq!(
        vec![
            RawItem {
...
async fn fetch_feed_items_atom() {
    let items = syndication::fetch_feed_items(&fixture("hnrss-org-frontpage.atom"), None).await.unwrap();
    assert_eq!(
        vec![
            RawItem {
                title: "Hacker Smacker: Friend/foe individual writers on Hacker News".to_string(),
